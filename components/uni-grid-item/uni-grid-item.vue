<template>
	<view v-if="width" :style="{ width: width }" class="uni-grid-item" @click="bindClick">
		<view :class="{ border: showBorder, 'uni-grid-item__box-square': square, 'border-top': showBorder && index < column, 'uni-highlight': highlight }" :style="{ 'border-color': borderColor }" class="uni-grid-item__box">
...
<script>
	export default {
		...
		computed: {
			width(){
				return parseInt(750 / this.column) - 20 + 'upx';
			}
			},
		methods: {
			bindClick() {
