        ...
        <view class="list-cell" hover-class="uni-list-cell-hover" @click="bindClick">
            <view class="media-list" v-if="newitem.title">
                <view :class="[isImgRight?'media-image-right':'',isImgLeft?'media-image-left':'']">
                    <text :class="['media-title',isImgRight||isImgLeft?'media-title2':'']">{{newitem.title}}</text>
                    ...
                    <view v-if="showImg" :class="['image-section',isImgRight?'image-section-right':'',isImgLeft?'image-section-left':'']">
					   ...
					   <template v-if="newitem['vs']!=''">
						   <image :class="['image-list1',isImgRight||isImgLeft?'image-list2':'']" v-if="ismp3"
                ...
                <view class="media-foot">
                    <view class="media-info">
                        ...
                        <text class="info-text">{{timestr}}</text>
                    </view>
                   
					<view class="max-close-view">
					<uni-icon v-if="newitem['mode']=='2'" size="30" type="download" style="color:#FF0000"></uni-icon>
					<template v-else>
					<uni-icon v-if="newitem['mode']=='3'" size="30" type="videocam" style="color:#FF0000"></uni-icon>
					</template>
					                        </view>
					                    </view>
...
<script>
    ...
    export default {
        ...
        computed: {
			...
			ismp3() {
				if(this.newitem['vs'].toLowerCase().indexOf('.mp3')) return true
			    return false
			},
            showImg() {
