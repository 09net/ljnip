<template>
    ...
    <view>
		...
		<view class="row" v-if="postno">
			<view class="col">
			<view class="col_3"><uni-icon size="30" type="comments" @click="opendiv"></uni-icon></view>
			<view class="col_3">
				
				<uni-icon size="30" type="heart" @click="fav" :color="favstart?'red':''"></uni-icon>
				</view>
			<view class="col_3">
				<uni-icon size="30" type="hand-grab-o" @click="shang" :color="shangstart?'red':''">赏</uni-icon>
	        </view>
			<view class="col_3">
				<uni-icon size="30" type="bars" @click="more"></uni-icon>
			</view>
		  </view>
		</view>
...
postno:true,
        ...
        methods: {
			more:function(){
				uni.showActionSheet({
					itemList: ['顶', '踩', '举报','赞赏(G100)','购买(G400)','评论'],
					success: (e) => {
						if(e.tapIndex=='3'){this.shang();return false;}
						if(e.tapIndex=='4'){this.fav();return false;}
						if(e.tapIndex=='5'){uni.navigateTo({url: 'com?mode='+mask.postactive+'&id='+this.thread['id']+'&uid='+this.thread['uid']});return false;}
						uni.request({method:'POST',
						url: 'https://'+ysv8set.website+'.44api.com/ebook/vote.api',
						data: {
						id:this.thread['id'],
					...
					shang:function(){
						...
						uni.request({method:'POST',
												url: ysv8set.www+mask.postactive+'/shang.api',
																							data: {
																								uid:this.thread['uid'],
																								tid:this.thread['id'],
																								pid:0,
																...
																mask.datacom=mask.add_array(this.thread,mask.datacom,false);
																ysv8set.localStorage.setItem("datacom"+mask.apiid,JSON.stringify(mask.datacom));
						},
						fail: (err) => {
																								uni.showModal({
																									content:err.errMsg,
					...
					fav:function(){
						...
						uni.request({method:'POST',
												url: ysv8set.www+'ebook/'+this.thread['id']+'/zip.api',
																							data: {
					...
					postsub:function(e){
						var formData = e.detail.value;				
							uni.request({method:'POST',
						url: ysv8set.www+mask.postactive+'/'+this.thread['id']+'.api',
																	data: {
									...
									uni.redirectTo({
										url: 'com?mode='+ mask.postactive +'&id='+this.thread['id']
									});
									
									
										}
										mask.add_array(this.thread,mask.datacom,false);
										ysv8set.localStorage.setItem("datacom"+mask.apiid,JSON.stringify(mask.datacom));
},
fail: (err) => {
																		uni.showModal({
